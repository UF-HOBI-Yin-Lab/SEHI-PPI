...
def data_read(data_path, end_str='new.csv'):
    for file in os.listdir(data_path):
        if file.endswith(end_str):
            ...
            # df = df[:300]
            pos_data, neg_pool, max_len = data_statics(df)
    return pos_data, neg_pool, max_len
...
def prepro_data(data_path, neg_type, data_name, end_str='new.csv', kmers=5, emb_file='./checkpoints/Pair_feature/', kmer_file='./Dataset/'):
    ...
    def data2kmers(pos_data, neg_data, kmers):
        ...
        pos_vir_kmer_str_list, pos_hos_kmer_str_list, pos_lbls = [], [], []
        for vir, hos, lbl in zip(pos_data[0], pos_data[1], pos_data[2]):
            flag = True
            vir_tem, hos_tem = [], []
            for i in range(len(vir)-kmers+1):
                kmer_str = vir[i:i + kmers]
                if kmer_str not in kmers_dict:
                    flag = False
                    break
                else:
                    vir_tem.append(kmer_str)
            for i in range(len(hos)-kmers+1):
                kmer_str = hos[i:i + kmers]
                if kmer_str not in kmers_dict:
                    flag = False
                    break
                else:
                    hos_tem.append(kmer_str)
            if not flag:
                continue
            else:
                pos_vir_kmer_str_list.append(vir_tem)
                pos_hos_kmer_str_list.append(hos_tem)
                pos_lbls.append(lbl)
        pos_kmers = (pos_vir_kmer_str_list, pos_hos_kmer_str_list, pos_lbls)
        
        neg_vir_kmer_str_list, neg_hos_kmer_str_list, neg_lbls = [], [], []
        for vir, hos, lbl in zip(neg_data[0], neg_data[1], neg_data[2]):
            # print('vir', vir, 'hos', hos)
            flag = True
            vir_tem, hos_tem = [], []
            for i in range(len(vir)-kmers+1):
                kmer_str = vir[i:i + kmers]
            ...
            if not flag:
                ...
                neg_vir_kmer_str_list.append(vir_tem)
                neg_hos_kmer_str_list.append(hos_tem)
                neg_lbls.append(lbl)
        neg_kmers = (neg_vir_kmer_str_list, neg_hos_kmer_str_list, neg_lbls)
        # print('neg_kmers', neg_kmers[1])
    ...
    def get_neg_data(pos_data, neg_pool, kmers_embs, neg_type, kmers=5):#  seq_sim
        ...
        for vir, hos in zip(pos_data[0], pos_data[1]):
            try:
                ...
                # print('vir', vir, 'all_negs', all_negs)
                ...
                if neg_type == 'random':
                    ...
                    # print(f'virus: {vir}, host: {hos}, negs_sel: {negs_sel}')
                    hos_kmers = [hos[i:i + kmers] for i in range(len(hos)-kmers+1)]
                    hos_kmers_emb = np.mean([kmers_embs[kmers_dict[kmer]] for kmer in hos_kmers if kmer in kmers_dict], axis=0)
                    # hos_kmers_emb = np.sum([kmers_embs[kmers_dict[kmer]] for kmer in hos_kmers], axis=0)
                    # print('negs_sel', negs_sel)
                    sel_negs_kmers_dict = {neg: [neg[i:i + kmers] for i in range(len(neg)-kmers+1)] for neg in negs_sel}
                    order_dict = OrderedDict()
                    for neg, nkmers in sel_negs_kmers_dict.items():
                        nkmers_emb = np.mean([kmers_embs[kmers_dict[kmer]] for kmer in nkmers if kmer in kmers_dict], axis=0) #make a big diff
                        # nkmers_emb = np.sum([kmers_embs[kmers_dict[kmer]] for kmer in nkmers], axis=0)
                        sim = cosine_similarity([hos_kmers_emb], [nkmers_emb])[0][0]
...
# def main():
#     raw_dir=f'./Dataset/VirusData/{params.data_name}/'
#     neg_type = params.neg_type
#     seed = params.seed
#     kmers = params.k
#     kmer_data, kmers_dict, max_seq_len = prepro_data(raw_dir, neg_type, params.source_data_name, end_str='new.csv', kmers=kmers)
#     print('max_seq_len:', max_seq_len, '\nkmer_data:', len(kmer_data[1]))
...
class ProteinDataset(DGLDataset):
    ...
    def process(self):
        ...
        # print('Loading raw data, kmer data and kmers dict...')
        ##########update2#########
        # kmer_data, self.kmers_dict, self.max_seq_len = prepro_data(self.raw_dir, self.neg_type, end_str='new.csv', kmers=self.kmers)
        kmer_data, self.kmers_dict, self.max_seq_len = prepro_data(self.raw_dir, self.neg_type, params.source_data_name, end_str='new.csv', kmers=self.kmers)
        ##########update2#########         
        # # Get labels and k-mer sentences
        self.k_virus, self.k_host, self.labels = kmer_data[0], kmer_data[1], torch.tensor(kmer_data[2])
        # print(f'vir:{self.k_virus}, hos:{self.k_host}, lbl:{self.labels}')
        ###########Graph Data############
        self.idSeq_virus = []
        ...
        self.graph1s = []
        self.graph2s = []
        for i in range(len(self.idSeq_virus)):  # 对于index列中的每一个sequence
            newidSeq_vir = []  # 对于每一个idseq,这个list从0一直往上增加，如果是之前出现过的则会返回到之前kmer对应的顺序index
            newidSeq_hos = []
            old2new_vir = {}  # 生成一个字典，里面的元素都是各不相同的
            old2new_hos = {}
            count_vir = 0
            count_hos = 0
            ...
            counter_uv = Counter(list(zip(newidSeq_vir[:-1], newidSeq_vir[1:])))  # 除去最后一个，除去第一个, 这步就是在构造De Brujin Graph了， Counter生成一个后面edge的频数字典
            ...
            counter_uv = Counter(list(zip(newidSeq_hos[:-1], newidSeq_hos[1:])))  # 除去最后一个，除去第一个, 这步就是在构造De Brujin Graph了， Counter生成一个后面edge的频数字典
         ...
         ###########Sequence Data############
        # print(type(self.idSeq_virus[0]), self.idSeq_virus[0])
        # print('self.idSeq_virus', self.idSeq_virus)
        ###########Sequence Data############
        pad_vir_seqs, len_vir_seqs = self.pad_and_cut(self.idSeq_virus, self.max_seq_len, seqt = 'vir')
        pad_hos_seqs, len_hos_seqs = self.pad_and_cut(self.idSeq_host, self.max_seq_len, seqt= 'hos')
        
        # print('len_vir_seqs', len_vir_seqs, 'len_hos_seqs', len_hos_seqs)
    ...
    def pad_and_cut(self, seqs, max_len, seqt):
        len_seqs = []
        padded_seqs = []
        if seqt == 'vir':
            for seq in seqs:
                seq_len = len(seq)
                len_seqs.append(seq_len)
                if  seq_len < max_len:
                    # 进行填充
                    new_seq = np.pad(seq, (0, max_len-seq_len), 'constant', constant_values=(0))
    ...
    def vectorize(self, method="bert", feaSize=params.d, loadCache=True):
        ...
        if os.path.exists(
            ##########update3#########
            #     f'checkpoints/Pair_feature/{method}_k{self.kmers}_d{feaSize}_embs.pkl') and loadCache:  # 这一步是看是否存在pkl文件，有的话就直接读取
            # with open(f'checkpoints/Pair_feature/{method}_k{self.kmers}_d{feaSize}_embs.pkl', 'rb') as f:
            ##########update3#########
                f'checkpoints/Pair_feature/{params.source_data_name}_{method}_k{self.kmers}_d{feaSize}_embs.pkl') and loadCache:  # 这一步是看是否存在pkl文件，有的话就直接读取
            with open(f'checkpoints/Pair_feature/{params.source_data_name}_{method}_k{self.kmers}_d{feaSize}_embs.pkl', 'rb') as f:
                if method == 'word2vec':
            ...
            # print(f'Load cache from checkpoints/Pair_feature/{method}_k{self.kmers}_d{feaSize}_embs.pkl!')
            return