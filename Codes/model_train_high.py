...
import numpy as np
import random
from utils.data_single_high import *
from models.model_high import *
from utils.config_high import *
import datetime
...
class Trainer:
    def __init__(self, model, loaders, optimizer, scheduler, lossfn, params):
        ...
        self.snapshot_path = f"%s{self.params.data_name}_{self.params.neg_type}_{self.params.g_model}_cf{self.params.fold}_model.pt" % params.savePath
    ...
    def train(self):
        print('Data name:', self.params.data_name, ', Neg Type:', self.params.neg_type, ', gnn:', self.params.g_model, ', gpu:', self.params.gpu_mode, ', batch:', self.params.batchSize, ', epoch:', self.params.num_epochs, \
    ...
    def train_epoch(self, train_dataloader, model, loss_fn, optimizer, gpu_id):
        ...
        epoch_loss = 0.0
        num_batches = len(train_dataloader)#g1, h1, g2, h2
        for _, (batched_graph_virus, batched_graph_host, _, _, _, _, labels) in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            # print('model before')
            output = model(batched_graph_virus, feats1, batched_graph_host, feats2)
            # print('model after')
    ...
    def valid_epoch(self, val_dataloader, model, loss_fn, gpu_id):
        ...
        with torch.no_grad():
            for _, (batched_graph_virus, batched_graph_host, _, _, _, _, labels) in enumerate(val_dataloader):
                # batched_graph_virus, batched_graph_host, seq_virus, seq_host, labels = batched_graph_virus.cuda(gpu_id), batched_graph_host.cuda(gpu_id), seq_virus.cuda(gpu_id), seq_host.cuda(gpu_id), labels.cuda(gpu_id)
                # seq_virus, seq_host, labels = seq_virus.cuda(gpu_id), seq_host.cuda(gpu_id), labels.cuda(gpu_id)
                labels = labels.cuda(gpu_id)
                feats1 = batched_graph_virus.ndata['attr'].cuda(gpu_id)
    ...
    def test(self, test_dataloader, model, gpu_id):
        model.eval()
        pred_list, label_list = [], []
        with torch.no_grad():
            for _, (batched_graph_virus, batched_graph_host, _, _, _, _, labels) in enumerate(test_dataloader):
                # batched_graph_virus, batched_graph_host, seq_virus, seq_host, labels = batched_graph_virus.cuda(gpu_id), batched_graph_host.cuda(gpu_id), seq_virus.cuda(gpu_id), seq_host.cuda(gpu_id), labels.cuda(gpu_id)
                # seq_virus, seq_host, labels = seq_virus.cuda(gpu_id), seq_host.cuda(gpu_id), labels.cuda(gpu_id)
                labels = labels.cuda(gpu_id)
                feats1 = batched_graph_virus.ndata['attr'].cuda(gpu_id)
...
def load_model_objs(params):
    in_dim, hidden_dim, output_dim, n_classes, g_model, device  = params.d, \
    params.hidden_dim, params.output_dim, params.n_classes, params.g_model, params.device

    model = PPINet(in_dim, hidden_dim, output_dim, n_classes, device)
