import torch
import os
from utils.data_single import *
from models.model_single_full import *
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.epochs_run, self.params.num_epochs):
            train_loss, train_acc, train_f, train_roc, train_sen, train_spe, train_mcc = self.train_epoch(train_dataloader, self.model, self.lossfn, self.optimizer, self.gpu_id)
            val_loss, val_acc, val_f, val_roc, val_sen, val_spe, val_mcc = self.valid_epoch(val_dataloader, self.model, self.lossfn, self.gpu_id)
            ...
            print(
                ...
                "Valid Acc:{:.3f}, Valid F1-score:{:.3f}, Valid ROC:{:.3f}, Valid Sensitivity:{:.3f}, Valid Specificity:{:.3f}, Valid Matthews Coef:{:.3f}!!!\n".format(
                    ...
                    val_acc, val_f, val_roc, val_sen, val_spe, val_mcc))
            if best_f < val_f:
                nobetter = 0
                best_f = val_f
        ...
        print("Finally,the model's Valid Acc:{:.3f}, Valid F1-score:{:.3f}, Valid ROC:{:.3f}, Valid Sensitivity:{:.3f}, Valid Specificity:{:.3f}, Valid Matthews Coef:{:.3f}!!!\n\n\n".format(
                ...
                best_record['valid_roc'], best_record['valid_sen'], best_record['valid_spe'], best_record['valid_mcc']))
    ...
    def train_epoch(self, train_dataloader, model, loss_fn, optimizer, gpu_id):
        ...
        train_acc, train_f, train_roc, train_sen, train_spe, train_mcc = accuracy(labels_list, preds_list), f1(labels_list, preds_list), auc(labels_list, preds_list), sensitivity(labels_list, preds_list), specificity(labels_list, preds_list), mcc(labels_list, preds_list)
    ...
    def valid_epoch(self, test_dataloader, model, loss_fn, gpu_id):
        ...
        labels_list = np.array(label_list)
        preds_list = np.argmax(pred_array, axis=1)
        valid_acc, valid_f, valid_roc, valid_sen, valid_spe, valid_mcc = accuracy(labels_list, preds_list), f1(labels_list, preds_list), auc(labels_list, preds_list), sensitivity(labels_list, preds_list), specificity(labels_list, preds_list), mcc(labels_list, preds_list)
    ...
    def test(self, eval_dataloader, model, gpu_id):
        ...
        valid_acc, valid_f, valid_roc, valid_sen, valid_spe, valid_mcc = accuracy(labels_list, preds_list), f1(labels_list, preds_list), auc(labels_list, preds_list), sensitivity(labels_list, preds_list), specificity(labels_list, preds_list), mcc(labels_list, preds_list)
        print("The overall performance on the test data are Acc:{:.3f}, F1-score:{:.3f}, ROC:{:.3f}, Sensitivity:{:.3f}, Specificity:{:.3f}, Matthews Coef:{:.3f}!!!\n\n\n".format(
                    valid_acc, valid_f, valid_roc, valid_sen, valid_spe, valid_mcc))
...
def load_model_objs(params):
    data_name, batch_size, seq_type, in_dim, hidden_dim, output_dim, n_classes, g_model, kmer, device  = params.data_name, params.batchSize, params.seq_type, params.d, \
    params.hidden_dim, params.output_dim, params.n_classes, params.g_model, params.k, params.device

    model = PPINet(data_name, kmer, seq_type, batch_size, in_dim, hidden_dim, output_dim, n_classes, g_model, device)
...
def main():
    ...
    dataset = ProteinDataset(raw_dir=f'./Dataset/{params.data_name}/', save_dir=f'checkpoints/vhgraph/k{params.k}_d{params.d}') #
